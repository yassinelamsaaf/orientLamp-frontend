...
const ChatAI = () => {
  ...
  const generateAIResponse = (userInput) => {
    ...
    if (input.includes('inpt') || input.includes('institut national des postes')) {
    ...
    if (input.includes('ensias') || input.includes('école nationale supérieure d\'informatique')) {
    ...
    if (input.includes('orientation') || input.includes('choix') || input.includes('filière') || input.includes('conseil')) {
      ...
      <div className="flex flex-col h-full bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800">
        ...
        <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-4 py-4 shadow-sm">
          <div className="flex items-center gap-3">
            <Avatar className="h-10 w-10 ring-2 ring-orientlamp-gold ring-offset-2">
              <AvatarImage src="/images/20b9a1b1-1802-42e7-8347-83c1f1324f09.png" alt="OrientLamp" />
              <AvatarFallback className="bg-orientlamp-gold text-white">OI</AvatarFallback>
            </Avatar>
            <div>
              <h1 className="text-lg font-bold text-gray-900 dark:text-white">OrientIA</h1>
              <p className="text-xs text-gray-500 dark:text-gray-400">Votre assistant d'orientation en ligne</p>
            </div>
          </div>
        </div>
            ...
            >
              {message.sender === 'ai' && (
                <Avatar className="mr-3 mt-1 h-8 w-8 ring-2 ring-gray-200 dark:ring-gray-700">
                  <AvatarImage src="/images/20b9a1b1-1802-42e7-8347-83c1f1324f09.png" alt="OrientLamp" />
                  <AvatarFallback className="bg-orientlamp-gold text-white text-xs">OI</AvatarFallback>
                </Avatar>
              )}
              <div 
              ...
              >
                ...
                <div className={`text-xs mt-2 ${
                  message.sender === 'user' ? 'text-amber-100' : 'text-gray-400 dark:text-gray-500'
                }`}>
              ...
              {message.sender === 'user' && (
                <Avatar className="ml-3 mt-1 h-8 w-8 ring-2 ring-orientlamp-gold">
                  <AvatarFallback className="bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-xs">U</AvatarFallback>
                </Avatar>
              )}
            </div>
          ))}
        ...
        <div className="bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 p-4 shadow-lg">
          ...
          <div className="text-xs text-gray-500 dark:text-gray-400 mt-3 text-center">
            💡 Essayez: "Qu'est-ce que l'INPT ?", "Comment choisir mon orientation ?"
          </div>
        </div>
      </div>
    </Layout>
