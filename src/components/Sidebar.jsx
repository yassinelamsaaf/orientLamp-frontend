...
import { Home, Search, Calendar, MessageSquare, User, Settings, LogIn, Sun, Moon, ChevronLeft, ChevronRight } from 'lucide-react';
  ...
  return (
    <div className="relative w-full md:w-auto h-auto md:h-screen flex">
      <div className={cn(
        ...
        <div className={cn("mb-6 flex items-center", showText ? "px-4 justify-start" : "px-2 justify-center")}> 
          {!showLabels && <Logo size={isCompact ? "sm" : "md"} showText={showText} />}
        </div>
      
      <nav className={cn("flex flex-col gap-2 flex-1", showText ? "px-2" : "px-1")}>
        {links.map((link) => {
          ...
          return (
            <Link
              ...
              title={link.label}
            >
              <Icon size={20} className={cn(showText ? "min-w-[20px]" : "")} />
      ...
      <div className={cn("mt-auto py-4 border-t border-gray-200 dark:border-gray-700", showText ? "px-2" : "px-1")}>
        <Link
          to="/login"
          className={cn(
        ...
        {showText ? (
          ...
          >
            <div className="flex items-center pointer-events-none">
              ...
              <span className="ml-3">Mode {theme === 'dark' ? 'clair' : 'sombre'}</span>
            </div>
            <Switch
              checked={theme === 'dark'}
